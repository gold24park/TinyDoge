...
class Compressor:
    ...
    def __init__(self):
        ...
        pngquant_path = os.path.join(getRootPath(), 'pngquant')
        self.temp_path = os.path.join(tempfile.gettempdir(), 'tmp_image')
    ...
    def compress(self, avg_quality: int, image_files: list, image_file_signal):
        if avg_quality == 0:
        ...
        for index, image_file in enumerate(image_files):
            ...
            subprocess.run(
                ...
                shell=True)
            subprocess.run(
                fr'{self.quant_file} --floyd=0.45 --speed=1 --quality={max(self.q - 30, 20)}-{self.q} --strip --force -o {temp_files[1]} "{image_file.filename}"',
                shell=True)
            subprocess.run(
                fr'{self.quant_file} --floyd=0.2 --speed=1 --quality={min(self.q + 10, 100)} --strip --force -o {temp_files[2]} "{image_file.filename}"',
                shell=True)
